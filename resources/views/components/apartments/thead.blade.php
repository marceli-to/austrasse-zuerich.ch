<thead class="font-semi font-normal sticky top-154 sm:relative sm:top-0 bg-serene">
  <tr class="[&>th]:py-6 [&>th]:pr-6 xs:[&>th]:pr-8 [&>th]:align-middle [&>th]:text-left [&>th]:leading-[1rem] [&>th]:w-auto [&>th]:font-semi [&>th]:font-normal border-b border-olive">
    <th>Nr.</th>
    <th>Etage</th>
    <th>Zi</th>
    <th class="">
      <span class="hidden xs:block">HNF/m<sup>2</sup></span>
      <span class="xs:hidden">m<sup>2</sup></span> 
    </th>
    <th class="!text-right !pr-10 hidden xs:table-cell">Netto<br>MZ/Mt.</th>
    <th class="!text-right !pr-10 hidden xs:table-cell">NK</th>
    <th class="!text-right !pr-18 hidden xs:table-cell">Brutto<br>MZ/Mt.</th>
    <th class="!text-right xs:hidden">Brutto/Mt.</th>
    <th class="text-center xs:text-left px-4 !pr-0 xs:!pr-8">
      <span class="hidden xs:block">Grundriss</span>
      <span class="xs:hidden">PDF</span>
    </th>
    <th class="!text-right !pr-0">Anmeldung</th>
  </tr>
</thead>